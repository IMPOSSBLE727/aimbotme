...
local cfgFile = "GHOSTS-Aimbot/config.json";
...
local function getCfgPath(name)
	return "GHOSTS-Aimbot/configs/" .. cleanCfgName(name) .. ".json";
end;
local function saveMeta()
	if not writefile or (not HS) then
		return;
	end;
	if isfolder and (not isfolder("GHOSTS-Aimbot")) then
		if makefolder then
			pcall(makefolder, "GHOSTS-Aimbot");
		else
			return;
		end;
	end;
...
local function getConfigNames()
	...
	if listfiles and isfolder and isfolder("GHOSTS-Aimbot/configs") then
		for _, path in listfiles("GHOSTS-Aimbot/configs") do
...
local UI = {
	...
	acc = Color3.fromRGB(120, 160, 220),
	acc2 = Color3.fromRGB(100, 150, 220),
	acc3 = Color3.fromRGB(140, 100, 200),
	ok = Color3.fromRGB(60, 180, 120),
	warn = Color3.fromRGB(220, 170, 60),
	danger = Color3.fromRGB(220, 80, 90),
	fallback = Color3.fromRGB(120, 160, 220)
};
local uiRefs = {};
...
local function applyTheme(name)
	local themes = {
		["Midnight Purple"] = { Color3.fromRGB(120, 160, 220), Color3.fromRGB(100, 150, 220), Color3.fromRGB(140, 100, 200) },
		Cyan = { Color3.fromRGB(80, 180, 220), Color3.fromRGB(60, 200, 240), Color3.fromRGB(140, 100, 200) },
		Purple = { Color3.fromRGB(140, 120, 220), Color3.fromRGB(160, 100, 240), Color3.fromRGB(140, 100, 200) },
		Pink = { Color3.fromRGB(220, 120, 180), Color3.fromRGB(240, 100, 200), Color3.fromRGB(120, 100, 220) },
		Green = { Color3.fromRGB(80, 200, 140), Color3.fromRGB(60, 220, 160), Color3.fromRGB(60, 200, 240) },
		Gold = { Color3.fromRGB(220, 180, 80), Color3.fromRGB(240, 200, 60), Color3.fromRGB(220, 120, 80) },
		Red = { Color3.fromRGB(220, 100, 110), Color3.fromRGB(240, 100, 100), Color3.fromRGB(220, 180, 60) },
		Glass = { Color3.fromRGB(120, 160, 220), Color3.fromRGB(100, 150, 220), Color3.fromRGB(140, 100, 200) }
	};
	local picked = tostring(name or "Glass");
	local t = themes[picked] or themes["Glass"];
	_G.uiTheme = themes[picked] and picked or "Glass";
	UI.acc = t[1];
	UI.acc2 = t[2];
	UI.acc3 = t[3];
	UI.stroke = Color3.fromRGB(255, 255, 255);
	UI.fallback = t[1];
end;
...
local function applyCustomUI()
	...
	_G.uiRounded = math.clamp(tonumber(_G.uiRounded) or 18, 10, 34);
	...
	if frm then
		frm.BackgroundTransparency = 0.65;
	end;
	if uiRefs.content then
		uiRefs.content.BackgroundTransparency = 0.6;
	end;
	...
	if uiRefs.topToggle then
		...
		uiRefs.topToggle.BackgroundTransparency = 0.5;
	end;
	if gui then
...
local function styleWin(frame, bar, title, btnX, btnMin)
	...
	noGrad(frame);
	local glassOverlay = newUi("ImageLabel", frame);
	setUiTag(glassOverlay, "GlassOverlay");
	glassOverlay.Size = UDim2.new(1, 0, 1, 0);
	glassOverlay.BackgroundTransparency = 1;
	glassOverlay.Image = "rbxassetid://10849683935";
	glassOverlay.ImageColor3 = UI.bg1;
	glassOverlay.ImageTransparency = 0.3;
	glassOverlay.ZIndex = 4;
	glassOverlay.ClipsDescendants = true;
	round(glassOverlay, UDim.new(0, _G.uiRounded or 24));
	...
	local function winBtn(b, base)
		...
		stroke(b, 1, Color3.fromRGB(0, 0, 0), 0.5);
		local icon = b:FindFirstChildOfClass("TextLabel");
		if icon then icon.TextColor3 = base; end;
		local e = b.MouseEnter:Connect(function()
			tw(b, TweenInfo.new(0.15, Enum.EasingStyle.Quart), {
...
VLO.aimOrigin.saveCfg = function(forceSave, nameOverride)
	...
	if isfolder and (not isfolder("GHOSTS-Aimbot")) then
		if makefolder then
			local s, e = pcall(makefolder, "GHOSTS-Aimbot");
	...
	if isfolder and (not isfolder("GHOSTS-Aimbot/configs")) then
		if makefolder then
			local s = pcall(makefolder, "GHOSTS-Aimbot/configs");
			if not s then
				return;
			end;
		else
...
local function createUI()
	...
	stroke(topBtn, 1, Color3.fromRGB(255, 255, 255), 0.3);
	uiRefs.updateLockUi();
	...
	noGrad(frame);
	local glassOverlay = newUi("ImageLabel", frame);
	setUiTag(glassOverlay, "RootGlass");
	glassOverlay.Size = UDim2.new(1, 0, 1, 0);
	glassOverlay.BackgroundTransparency = 1;
	glassOverlay.Image = "rbxassetid://10849683935";
	glassOverlay.ImageColor3 = UI.bg1;
